...
export class AIBotService {
    ...
    try {
      ...
      try {
        const knownAliases = await this.database.query(
          'SELECT * FROM user_memory ORDER BY memory_quality DESC'
        ) as UserMemory[];
  ...
  private async resolveUserName(chatId: string, mentionedName: string): Promise<{
    ...
    try {
      ...
      const matches: { alias: UserMemory; score: number }[] = [];
  ...
  async createUserAlias(
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        await this.database.query(
          'UPDATE user_memory SET aliases = $1, real_name = $2, memory_quality = $3, last_updated = NOW() WHERE user_id = $4',
          [JSON.stringify(mergedAliases), realName, confidence, userId]
        );
      } else {
        // Create new alias
        await this.database.query(
          'INSERT INTO user_memory (user_id, real_name, aliases, memory_quality, created_by) VALUES ($1, $2, $3, $4, $5)',
